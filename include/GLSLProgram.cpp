...
namespace ZEngine {
    ...
    void GLSLProgram::compileShaders(const QString& vertexShaderFilePath, const QString& fragmentShaderFilePath) {
        QFile vertFile(vertexShaderFilePath);
        QFile fragFile(fragmentShaderFilePath);
        vertFile.open(QIODevice::ReadOnly);
        fragFile.open(QIODevice::ReadOnly);
        compileShadersFromSource((const char*)vertFile.readAll(), (const char*)fragFile.readAll());
        vertFile.close();
        fragFile.close();
    }
